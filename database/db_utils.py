...
# ---------------------------------------------------------------------------
# possible_matches helpers
# ---------------------------------------------------------------------------

def make_group_fingerprint(products: list[dict]) -> str:
    """
    Return a stable 32-char MD5 fingerprint for a candidate match group.

    The fingerprint is computed from sorted "source_name:product_url" tokens,
    one per product in the group.  Using product_url (not the DB id) makes it
    stable across full database re-imports: as long as the source website URLs
    don't change, the same candidate will always produce the same fingerprint
    and the UNIQUE constraint on possible_matches.group_fingerprint will
    prevent it from being re-queued after a scrape+re-import cycle.

    Args:
        products: list of dicts, each must have 'source_name' and 'product_url'.

    Returns:
        32-character lowercase hex string (MD5).

    Example:
        >>> make_group_fingerprint([
        ...     {"source_name": "ecuga",    "product_url": "https://ecuga.com/p/1"},
        ...     {"source_name": "cugaklik", "product_url": "https://cugaklik.hr/p/2"},
        ... ])
        'a3f8...'
    """
    tokens = sorted(
        f"{p['source_name']}:{p['product_url']}" for p in products
    )
...
def extract_volume_ml(title: str) -> Optional[int]:
    ...
    Examples:
        ...
        "Vodka 1.75l" → 1750
        "Rum 375ml" → 375
        "12 x 0,05 L" → 600   (multi-pack: 12 × 50ml)
        "24 x 33cl"   → 7920  (multi-pack: 24 × 330ml)
        "120 x 0,05l" → 6000  (multi-pack: 120 × 50ml)
    """
    title_lower = title.lower()

    # Match volume patterns with priority order (most specific first)
    patterns = [
        # Multi-pack: "12 x 0,05 L", "6 x 700ml", "24 x 33cl", "120 x 0,05l"
        # Total volume = count × unit_volume
        (r'(\d+)\s*[x×]\s*(\d+)[,.](\d{3})\s*l\b', lambda m: int(m.group(1)) * (int(m.group(2)) * 1000 + int(m.group(3)))),
        (r'(\d+)\s*[x×]\s*(\d+)[,.](\d{2})\s*l\b',  lambda m: int(m.group(1)) * (int(m.group(2)) * 1000 + int(m.group(3)) * 10)),
        (r'(\d+)\s*[x×]\s*(\d+)[,.](\d)\s*l\b',     lambda m: int(m.group(1)) * (int(m.group(2)) * 1000 + int(m.group(3)) * 100)),
        (r'(\d+)\s*[x×]\s*(\d+)\s*l\b',             lambda m: int(m.group(1)) * int(m.group(2)) * 1000),
        (r'(\d+)\s*[x×]\s*(\d+)\s*ml\b',            lambda m: int(m.group(1)) * int(m.group(2))),
        (r'(\d+)\s*[x×]\s*(\d+)\s*cl\b',            lambda m: int(m.group(1)) * int(m.group(2)) * 10),
        # Decimal liters with 3 digits: 0.375l, 0,355l, 1.750l
        (r'(\d+)[,.](\d{3})\s*l\b', lambda m: int(m.group(1)) * 1000 + int(m.group(2))),
        # Decimal liters with 2 digits: 0.75l, 0,75l, 1.75l
        (r'(\d+)[,.](\d{2})\s*l\b', lambda m: int(m.group(1)) * 1000 + int(m.group(2)) * 10),
        # Decimal liters with 1 digit: 0.7l, 0,7l, 1.5l (most common)
        (r'(\d+)[,.](\d)\s*l\b', lambda m: int(m.group(1)) * 1000 + int(m.group(2)) * 100),
        # Whole liters: 1l, 2l, 3l
        (r'(\d+)\s*l\b', lambda m: int(m.group(1)) * 1000),
        # Milliliters: 700ml, 500ml, 375ml, 50ml
        (r'(\d+)\s*ml\b', lambda m: int(m.group(1))),
        # Centiliters: 70cl, 75cl, 50cl
    ...
    for pattern, converter in patterns:
        ...
        if match:
            try:
                volume = converter(match)
                # Sanity check: 10ml (shot glass) to 50L (largest multi-pack)
                if 10 <= volume <= 50000:
                    return volume
...
def normalize_title(title: str) -> str:
    ...
    # Strip ABV percentages early — "45,2%", "43.2%", "40 %" etc.
    # Without this, "45,2%" would later become "45 2" (two junk tokens) after
    # special-character stripping, polluting match keys.
    normalized = re.sub(r'\d+[,.]?\d*\s*%', '', normalized)

    # Remove "the" prefix
    normalized = re.sub(r'\bthe\s+', '', normalized)

    # Standardize volume format to consistent ml representation
    # This helps ensure "0.7l", "700ml", "70cl" all become "700ml"
    ...
    # Extract and standardize volumes to ml
    # We'll convert all to ml format for consistency

    # Decimal liters with 2 digits: 0.75l, 0,75l → 750ml
    normalized = re.sub(r'(\d+)[,.](\d{2})\s*l\b', lambda m: f"{int(m.group(1)) * 1000 + int(m.group(2)) * 10}ml", normalized)

    # Decimal liters with 1 digit: 0.7l, 0,7l → 700ml
    normalized = re.sub(r'(\d+)[,.](\d)\s*l\b', lambda m: f"{int(m.group(1)) * 1000 + int(m.group(2)) * 100}ml", normalized)

    # Whole liters: 1l, 2l → 1000ml, 2000ml
    normalized = re.sub(r'(\d+)\s*l\b', lambda m: f"{int(m.group(1)) * 1000}ml", normalized)

    # Centiliters: 70cl, 75cl → 700ml, 750ml
    normalized = re.sub(r'(\d+)\s*cl\b', lambda m: f"{int(m.group(1)) * 10}ml", normalized)

    # Milliliters are already in the right format, just remove spaces
    normalized = re.sub(r'(\d+)\s+ml\b', r'\1ml', normalized)
    ...
    # IMPORTANT: We now KEEP volume indicators (in standardized ml format) for better matching

    # Remove special characters but keep spaces and numbers (for volumes)
